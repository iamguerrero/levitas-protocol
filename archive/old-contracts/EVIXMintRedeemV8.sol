...
/**
 * @title EVIXMintRedeem V8 - Advanced Liquidation System
 * @notice Production-ready vault with permissionless liquidation, partial liquidation, and enhanced safety features
 * @dev Implements maker-style liquidation with bonus incentives for EVIX (Ethereum Volatility Index)
 */
contract EVIXMintRedeemV8 is AccessControl, ReentrancyGuard, Pausable {
        ...
        returns (uint256) 
    {
        ...
        uint256 tokensToMint = (debtValue * 1e18) / price; // Convert to 18 decimals

        if (tokensToMint == 0) revert InvalidAmount();

        // Check global CR after mint
        uint256 futureCollateral = totalCollateral + netAmount;
        uint256 futureDebtValue = ((totalDebt + tokensToMint) * price) / 1e18;
     ...
     * @param repayAmount Amount of EVIX to repay (0 for max liquidation)
     */
        ...
        whenNotPaused 
    {
        ...
        uint256 price = oracle.getPrice(); // 6 decimals
        uint256 debtValue = (pos.debt * price) / 1e18; // Convert EVIX to USDC value
        ...
        uint256 debtValueRepaid = (debtToRepay * price) / 1e18;
        ...
        // Transfer tokens
        evix.transferFrom(msg.sender, address(this), debtToRepay);
        evix.burn(address(this), debtToRepay);
    ...
    function getCollateralRatio() public view returns (uint256) {
        ...
        uint256 price = oracle.getPrice(); // 6 decimals
        uint256 debtValue = (totalDebt * price) / 1e18; // Convert EVIX to USDC value
    ...
    function getLiquidationPrice(address user) public view returns (uint256) {
        ...
        // Return price in 6 decimals
        return (pos.collateral * 100 * 1e18) / (pos.debt * liquidationThreshold);
    }
