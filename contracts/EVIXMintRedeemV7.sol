...
/**
 * @title EVIX MintRedeem V7 - Secure Position-Aware Vault with Access Control
 * @notice Production-ready vault with role-based access, reentrancy protection, and liquidation
 * @dev Integrates with secure PriceOracle for EVIX (Ethereum Volatility Index)
 */
contract EVIXMintRedeemV7 is AccessControl, ReentrancyGuard, Pausable {
    ...
    struct Position {
        ...
        uint256 debt;        // EVIX minted (18 decimals)
    }
        ...
        returns (uint256) 
    {
        ...
        uint256 price = oracle.getPrice(); // 6 decimals
        uint256 fee = (amount * mintFee) / 10000;
        uint256 netAmount = amount - fee;

        // Calculate debt based on target CR
        uint256 debtValue = (netAmount * 100) / targetCR; // 6 decimals
        uint256 tokensToMint = (debtValue * 1e30) / price; // Adjusted for price with 18 decimals (debtValue 6 dec * 1e30 / price 18 dec = tokens 18 dec)

        if (tokensToMint == 0) revert InvalidAmount();

        // Check global CR after mint
        uint256 futureCollateral = totalCollateral + netAmount;
        uint256 futureDebtValue = ((totalDebt + tokensToMint) * price) / 1e18; // Convert EVIX (18 decimals) to USDC equivalent (6 decimals)
    ...
    /**
     * @notice Redeem USDC by burning EVIX tokens
     * @param amount EVIX amount to burn (18 decimals)
        ...
        returns (uint256) 
    {
        ...
        uint256 price = oracle.getPrice(); // 6 decimals
        uint256 debtValue = (amount * price) / 1e18; // Convert EVIX (18 decimals) to USDC equivalent (6 decimals)
        ...
        onlyRole(LIQUIDATOR_ROLE) 
    {
        ...
        uint256 price = oracle.getPrice(); // 6 decimals
        uint256 debtValue = (pos.debt * price) / 1e18; // Convert EVIX (18 decimals) to USDC equivalent (6 decimals)
    ...
    function getCollateralRatio() public view returns (uint256) {
        ...
        uint256 price = oracle.getPrice(); // 6 decimals
        uint256 debtValue = (totalDebt * price) / 1e18; // Convert EVIX (18 decimals) to USDC equivalent (6 decimals)
    ...
    function getLiquidationPrice(address user) public view returns (uint256) {
        Position memory pos = positions[user];
        if (pos.debt == 0) return 0;
        return (pos.collateral * liquidationThreshold * 1e18) / (pos.debt * 100); // Return price in 6 decimals
    }
