...
async function main() {
  ...
  try {
    ...
    const BVIXV6 = await hre.ethers.getContractFactory("MintRedeemV6");
    ...
    const bvixToken = await hre.ethers.getContractAt("BVIXToken", BVIX_TOKEN_ADDRESS);
    await bvixToken.transferOwnership(bvixV6Address);
    console.log("✅ BVIX ownership transferred");

    // Deploy EVIX V6
    console.log("\n🏗️ Deploying EVIX MintRedeem V6...");
    const EVIXV6 = await hre.ethers.getContractFactory("EVIXMintRedeemV6");
    ...
    const evixToken = await hre.ethers.getContractAt("EVIXToken", EVIX_TOKEN_ADDRESS);
