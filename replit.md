...
✅ **FRESH VAULT LOGIC APPLIED TO BOTH TOKENS** - Implemented consistent fresh vault detection for BVIX and EVIX using contract state tracking at liquidation time. Both tokens now properly show only post-liquidation minting activity (300 USDC/6 BVIX at 118.62% CR) while hiding pre-liquidation cumulative data.
