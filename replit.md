...
🔧 **VAULT ISOLATION ISSUE IDENTIFIED** - V8 BVIX contract incorrectly uses cumulative user positions instead of individual vault tracking. User wants each mint to create separate vault entity, not accumulate into one position. Smart contract design needs individual vault IDs with separate collateral/debt tracking per vault.
